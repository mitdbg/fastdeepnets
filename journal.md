...
- [x] Check the hypotesis made in the previous step
  - As we can see on this plot (generated by the function `validate_plateau_hypothesis` in `/simple_metricless.py`. This hypotesis is clearly valid. All the weigths associated to disabled neurons are zero. The l2 regularization is without any doubt the reason why this happens.
