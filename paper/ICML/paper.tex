...
\begin{abstract}
As neural networks are being widely in different applications and
...
optimizes model size, model accuracy and inference time in different stages, 
resulting in suboptimal results and computational inefficiency.  In
this work, we propose a new technique called ShrinkNets that optimizes all three
...
convolutional and fully connected layers, we show that ShrinkNets can reduce network size
by 35x in comparison to to a static network on \texttt{CIFAR10}, with
6x improvement in inference time. In addition, the
