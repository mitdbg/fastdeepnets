...
groups: $\bm{\theta} = \bigcup_{i=1}^P \bm{\theta}_i$, group sparsity is defined as: \srm{you used to say ``we define `it' as:'' -- I am assume `it' is group sparsity}
%
\begin{equation}
  \Omega_\lambda^{gp} = \lambda \sum_{i=1}^p \sqrt{\#\bm{\theta_i}} \norm{\bm{\theta_i}}_2 \\
\end{equation}
%
\srm{define notation -- what is $\lambda$;  what is $\#\theta$, what is $\Omega$? Where does the square root come from?}
...
original ShrinkNet problem is \srm{do you mean ``can be defined formally as'' -- we never gave this definition of ShrinkNets.  Also, you need to explain the formula / notation;  what is $y$, what is $\beta$, what is this equation representing (the optimization function?)}:
%
\begin{equation}
...
as \srm{where?}:\ra{the notation is slightly different, better to make it consistent}
%
\begin{equation}
...
will also introduce additional sparsity into the parameters. \srm{why?}
