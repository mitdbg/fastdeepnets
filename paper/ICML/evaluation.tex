...
substantially different than the best network architecutres (in terms of
...
\noindent\textbf{Implementation: }We have implemented \textbf{switch Layers} and
...
\subsubsection{Large Network Space: \texttt{CIFAR10}}
...
\subsubsection{Small Network Space: \texttt{COVERTYPE}}
...
We have demonstrated that it is possible to achieve networks with good accuracy
...
We have demonstrated that ShrinkNets find networks of similar or better accuracy
than static networks, and much smaller. Here we explore if the reduction in size
...
it implies having enormous models. This observation motivates our experimental
methodology: for a given desired target accuracy, we pick the smallest network
that satisfies the accuracy constraint. For both \textit{ShrinkNets} and
\textit{Static Networks} we measure multiple metrics and compute the ratio
between the two. The metrics we are interested in are: the final model size (in
...
Regarding size, the key observation is that size improvements are significant
for the \texttt{CIFAR10}. In the range of accuracies we are interested in,
improvements in size go from 4x to 40x. On the \texttt{COVERTYPE} dataset even
if the compression ratio is always above 1, it hardly exceeds 3x, except for
very high accuracy where \textit{ShrinkNets} obtained some particularly good
solutions.

We observe that unlike local sparsity compression based method, our method's
improvement on size translates directly to higher throughput at inference time.
\ra{For example, when running on CPU, blah, when running on GPU blah.}

When using a batch size of $1$ on CPU for \texttt{CIFAR10} as well as when using
a batch of $1$ on GPU, we do not observe any improvement due to the experiment
being dominated by noise. However, we still expect to find a difference in
latency if we were using a framework optimized for such goal.\ra{this is a bit
vague}
