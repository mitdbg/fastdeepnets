...
also sensible to gradient noise. \ra{explain what's the consequence of that} If
we sample a gradient that is not representative of the dataset when performing
gradient descent then we might wrongly kill a neuron.\ra{last sentence is
unclear}
...
layer by multiplying the respective parameters.  \ra{can't we just simply
prevent the following problem at network design time?}In the unlikely case that a switch
